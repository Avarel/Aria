...
class PlayCommand : Command<MessageContext> {
    override suspend operator fun invoke(context: MessageContext) {
        context.dsl {
            string(
                "(query | url)",
                "A search query or a music http URL.",
                true
            ) { query ->
                val controller = join(context, false) ?: return

                val list = context.bot.musicManager.search(
                    if ("https://" in query) query else "ytsearch:$query",
                    1
                )
                ...
                track.userData =
                    TrackContext(context.member!!, context.textChannel)

                context.channel.sendEmbed(track.info.title, track.info.uri) {
                    setAuthor(track.info.author)

                    field("Duration", true) {
                        Duration.ofMillis(track.duration).formatDuration()
                    }
                    field("Time Until Play", true) {
                        val duration =
                            (controller.player.playingTrack?.remainingDuration
                                ?: 0) - controller.scheduler.duration
